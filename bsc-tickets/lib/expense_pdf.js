...
function buildConveyancePDF(d) {
  return new Promise((resolve, reject) => {
    ...
    // table
    const cols = [
      { t: '#', w: 24, a: 'left' }, { t: 'Date', w: 64, a: 'left' },
      { t: 'From', w: 110, a: 'left' }, { t: 'To', w: 110, a: 'left' },
      { t: 'Vehicle', w: 78, a: 'left' }, { t: 'KM', w: 44, a: 'right' },
      { t: 'Amount', w: 78, a: 'right' },
    ];
    ...
    (d.entries || []).forEach((e, i) => {
      ...
      cx = tableX + 8;
      const vals = [String(i + 1), e.date, e.from, e.to, e.vehicle_label, String(e.km), rupee(e.amount).replace('INR ', '₹')];
      doc.fillColor(INK);
      cols.forEach((c, ci) => { doc.text(vals[ci], cx, y + 6, { width: c.w - 8, align: c.a, ellipsis: true }); cx += c.w; });
      y += 20;
    });
    // total row
    doc.rect(tableX, y, tableW, 24).fillColor('#EAF2F8').fill();
    doc.fillColor(INK).font('BodyBold').fontSize(11)
       .text('TOTAL', tableX + 8, y + 7, { width: tableW - 86 - 8 });
    doc.text(rupee(d.total).replace('INR ', '₹'), tableX + tableW - 86, y + 7, { width: 78, align: 'right' });
    y += 40;
...
const CATLABEL = { accommodation: 'Accommodation', food: 'Food', conveyance: 'Conveyance', others: 'Others' };
