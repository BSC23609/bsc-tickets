...
router.post('/ot/bulk', async (req, res) => {
  ...
  for (const e of (req.body.entries || [])) {
    const name = String(e.labour_name || '').trim(), date = String(e.ot_date || '').slice(0, 10), hours = parseFloat(e.hours);
    if (!name) continue;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) return res.status(400).json({ error: `Bad date for ${name}` });
    ...
    clean.push({ name, date, hours: +hours.toFixed(2), amount: otAmount(hours) });
  }
  if (!clean.length) return res.status(400).json({ error: 'Add at least one row with name, date and hours.' });
  const period = clean[0].date.slice(0, 7);
  if (!(await assertEditable(company, period, res))) return;
  let total = 0;
  for (const c of clean) {
    total += c.amount;
    await q(`INSERT INTO labour_ot(company,labour_name,ot_date,period,hours,amount,entered_by_id,entered_by_name)
             VALUES($1,$2,$3,$4,$5,$6,$7,$8)`, [company, c.name, c.date, c.date.slice(0, 7), c.hours, c.amount, req.user.id, req.user.name]);
  }
  await refreshTotals(company, period);
...
router.put('/ot/:id', async (req, res) => {
  ...
  const name = String(req.body.labour_name || '').trim(), date = String(req.body.ot_date || '').slice(0, 10), hours = parseFloat(req.body.hours);
  if (!name || !/^\d{4}-\d{2}-\d{2}$/.test(date) || !(hours > 0)) return res.status(400).json({ error: 'Name, date and hours are required.' });
  if (!(await assertEditable(row.company, date.slice(0, 7), res))) return;
  await q(`UPDATE labour_ot SET labour_name=$2,ot_date=$3,period=$4,hours=$5,amount=$6,updated_at=now() WHERE id=$1`,
    [row.id, name, date, date.slice(0, 7), +hours.toFixed(2), otAmount(hours)]);
  await refreshTotals(row.company, date.slice(0, 7));
  res.json({ ok: true });
});
...
async function buildReportHtml(company, month) {
  ...
  const ot = (await q(`SELECT labour_name,to_char(ot_date,'DD Mon') AS d,hours,amount FROM labour_ot WHERE company=$1 AND period=$2 ORDER BY labour_name,ot_date`, [company, month])).rows;
  ...
  const otRows = ot.map(r => `<tr><td ${td}>${r.labour_name}</td><td ${td}>${r.d}</td><td ${td}>${(+r.hours).toFixed(2)}</td><td ${td} align="right">${money(r.amount)}</td></tr>`).join('') || `<tr><td ${td} colspan="4">No OT entries.</td></tr>`;
  ...
  return `<div style="font-family:Segoe UI,Arial,sans-serif;color:#111;max-width:640px">
    ...
    <table style="border-collapse:collapse;width:100%"><tr><th ${th}>Name</th><th ${th}>Date</th><th ${th}>Hours</th><th ${th} align="right">Amount</th></tr>${otRows}</table>
