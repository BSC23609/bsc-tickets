...
function buildCoverPdf({ empName, empNo, monthLabel, breakdown, total }) {
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('#334155').font('BB').fontSize(15).text(`${empName}  \u00b7  ${empNo}`, 0, 100, { align: 'center', width: W });
    doc.fillColor('#8A97A3').font('B').fontSize(11).text(monthLabel, 0, 124, { align: 'center', width: W });
    doc.moveTo(M, 152).lineTo(W - M, 152).lineWidth(2).strokeColor('#0A4566').stroke();
    let y = 184;
