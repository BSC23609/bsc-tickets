...
// YYYY-MM-DD -> DD-MM-YY (leaves anything else untouched)
function fmtDMY(s) {
  if (!s) return '';
  const p = String(s).slice(0, 10).split('-');
  return p.length === 3 ? `${p[2]}-${p[1]}-${p[0].slice(2)}` : String(s);
}
function billCount(it) { return (it.bills && it.bills.length) ? it.bills.length : ((it.bill || it.bill_name) ? 1 : 0); }
...
function buildConveyancePDF(d) {
  return new Promise((resolve, reject) => {
    ...
    // header row (reusable so it repeats after a page break)
    const drawHead = (yy) => {
      doc.rect(tableX, yy, tableW, 22).fillColor(INK).fill();
      let hx = tableX + 8;
      doc.fillColor('#fff').font('BodyBold').fontSize(8);
      for (const c of cols) { doc.text(c.t.toUpperCase(), hx, yy + 7, { width: c.w - 8, align: c.a }); hx += c.w; }
      return yy + 22;
    };
    y = drawHead(y);
    // body — each row grows to fit its tallest wrapped cell so nothing overlaps
    let anyLate = false;
    (d.entries || []).forEach((e, i) => {
      const vals = [String(i + 1), fmtDMY(e.date), e.from || '-', e.to || '-', e.purpose || '-', e.vehicle_label,
        String(e.km), rupee(e.amount).replace('INR ', '₹'), e.late ? 'LATE' : ''];
      // measure the tallest cell at its column width -> row height
      doc.font('Body').fontSize(8.5);
      let textH = 0;
      cols.forEach((c, ci) => { const h = doc.heightOfString(String(vals[ci] || ''), { width: c.w - 8 }); if (h > textH) textH = h; });
      const rowH = Math.max(20, Math.ceil(textH) + 10);
      // start a fresh page (repeating the header) if this row would hit the footer band
      if (y + rowH > doc.page.height - 130) { doc.addPage(); y = 44; y = drawHead(y); }
      if (e.late) { anyLate = true; doc.rect(tableX, y, tableW, rowH).fillColor(REDBG).fill(); }
      else if (i % 2) { doc.rect(tableX, y, tableW, rowH).fillColor(MIST).fill(); }
      let cx = tableX + 8;
      cols.forEach((c, ci) => {
        if (c.t === 'Flag' && e.late) doc.fillColor(RED).font('BodyBold').fontSize(7.5);
        else doc.fillColor(INK).font('Body').fontSize(8.5);
        doc.text(String(vals[ci] || ''), cx, y + 5, { width: c.w - 8, align: c.a });
        cx += c.w;
      });
      y += rowH;
    });
    ...
    if (d.status === 'approved') {
      ...
      doc.font('Body').fontSize(10).fillColor('#EAFBF1')
         .text(`Approved by ${d.approver || 'the approver'}  ·  ${d.approved_at || ''}`, M + 150, bandY + 14, { width: bandW - 150 - 16, align: 'right' });
      ...
      doc.font('Body').fontSize(10).fillColor('#FCE9E9')
         .text(`Returned by ${d.approver || 'the approver'}  ·  ${d.approved_at || ''}`, M + 150, bandY + 14, { width: bandW - 150 - 16, align: 'right' });
    } else {
      ...
      doc.fillColor(GRAY).font('Body').fontSize(10)
         .text('Submitted to HR — pending approval.', M + 16, bandY + 15);
    }
    doc.end();
  });
}
...
function statusBand(doc, d, W, M, y) {
  ...
  if (d.status === 'approved') {
    ...
    doc.fillColor('#fff').font('BodyBold').fontSize(12).text('APPROVED', M + 16, bandY + 13);
    doc.font('Body').fontSize(10).fillColor('#EAFBF1')
       .text(`Approved by ${d.approver || 'the approver'}  ·  ${d.approved_at || ''}`, M + 150, bandY + 14, { width: W - 2 * M - 150 - 16, align: 'right' });
  } else if (d.status === 'rejected') {
    doc.roundedRect(M, bandY, W - 2 * M, 40, 8).fillColor(RED).fill();
    doc.fillColor('#fff').font('BodyBold').fontSize(12).text('NOT APPROVED', M + 16, bandY + 13);
    doc.font('Body').fontSize(10).fillColor('#FCE9E9')
       .text(`Returned by ${d.approver || 'the approver'}  ·  ${d.approved_at || ''}`, M + 150, bandY + 14, { width: W - 2 * M - 150 - 16, align: 'right' });
  ...
  return new Promise((resolve, reject) => {
    ...
    (d.trips || []).forEach((trip, ti) => {
      ...
      doc.fillColor(GRAY).font('Body').fontSize(8.5)
         .text(`${fmtDMY(trip.from_date)} → ${fmtDMY(trip.to_date)}    ·    ${trip.reason || ''}`, M + 12, y + 20, { width: tableW - 24, ellipsis: true });
      y += 42;
      drawHead();
      let sub = 0;
      (trip.items || []).forEach((it, i) => {
        ...
        doc.text(fmtDMY(it.date), tableX + cw.cat + 8, y + 5, { width: cw.date - 8 });
        const desc = (it.desc || '') + (billCount(it) > 1 ? `  [${billCount(it)} bills]` : (billCount(it) === 1 ? '  [bill]' : ''));
...
function buildMiscSummary(d) {
  return new Promise((resolve, reject) => {
    ...
    (d.items || []).forEach((it, i) => {
      ...
      doc.text(fmtDMY(it.date), tableX + 8, y + 6, { width: cw.date - 8 });
      doc.text((it.desc || '') + (billCount(it) > 1 ? `  [${billCount(it)} bills]` : (billCount(it) === 1 ? '  [bill]' : '')), tableX + cw.date + 8, y + 6, { width: cw.desc - 8, ellipsis: true });
