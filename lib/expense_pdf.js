  ...
  return new Promise((resolve, reject) => {
    ...
    if (d.status === 'approved') {
      ...
      doc.font('Body').fontSize(10).fillColor('#EAFBF1')
         .text(`Approved by ${d.approver || 'the approver'}    ${d.approved_at || ''}`, M, bandY + 14, { width: bandW - 16, align: 'right' });
      ...
      doc.font('Body').fontSize(10).fillColor('#FCE9E9')
         .text(`Returned by ${d.approver || 'the approver'}    ${d.approved_at || ''}`, M, bandY + 14, { width: bandW - 16, align: 'right' });
    } else {
...
function statusBand(doc, d, W, M, y) {
  ...
  if (d.status === 'approved') {
    ...
    doc.fillColor('#fff').font('BodyBold').fontSize(12).text('APPROVED', M + 16, bandY + 13);
    doc.font('Body').fontSize(10).fillColor('#EAFBF1')
       .text(`Approved by ${d.approver || 'the approver'}    ${d.approved_at || ''}`, M, bandY + 14, { width: W - 2 * M - 16, align: 'right' });
  } else if (d.status === 'rejected') {
    doc.roundedRect(M, bandY, W - 2 * M, 40, 8).fillColor(RED).fill();
    doc.fillColor('#fff').font('BodyBold').fontSize(12).text('NOT APPROVED', M + 16, bandY + 13);
    doc.font('Body').fontSize(10).fillColor('#FCE9E9')
       .text(`Returned by ${d.approver || 'the approver'}    ${d.approved_at || ''}`, M, bandY + 14, { width: W - 2 * M - 16, align: 'right' });
