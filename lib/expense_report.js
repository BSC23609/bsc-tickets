...
function effectivePeriod(r) {
  ...
  return r.form_type === 'misc' ? reportBucket(r.paid_at) : r.period;  // misc lands in the month it was PAID
}
...
async function reportData(period) {
  ...
  const all = (await q(`SELECT s.ref_no, s.form_type, s.total_amount, s.final_at, s.final_by_name, s.pdf_token,
      s.period, s.paid_at, s.report_period_override, e.name AS emp_name, e.emp_no
    FROM expense_submissions s JOIN employees e ON e.id=s.employee_id
    WHERE s.status='approved' AND (s.period=$1 OR s.form_type='misc' OR s.report_period_override IS NOT NULL)`, [period])).rows;
  const rows = all
    .filter(r => effectivePeriod(r) === period)
    .map(({ period: _p, paid_at: _pa, report_period_override: _o, ...r }) => r)
