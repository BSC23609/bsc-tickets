...
// Map any date/timestamp to its report period (IST). Pre-cutover = calendar month; cutover+ = 25->25 cycle.
function reportBucket(dateInput) {
  if (!dateInput) return null;
  const base = new Date(dateInput); if (isNaN(base)) return null;
  const ist = new Date(base.getTime() + 5.5 * 3600 * 1000);
  const y = ist.getUTCFullYear(), mo = ist.getUTCMonth(), day = ist.getUTCDate();
  const ym = `${y}-${String(mo + 1).padStart(2, '0')}`;
  if (ym < CUTOVER_PERIOD) return ym;
  let cy = y, cm = mo; if (day >= 26) { cm++; if (cm > 11) { cm = 0; cy++; } }
  return `${cy}-${String(cm + 1).padStart(2, '0')}`;
}
// Which report month a claim belongs to: manual override wins; misc = submission date; others = their period.
function effectivePeriod(r) {
  if (r.report_period_override) return r.report_period_override;
  return r.form_type === 'misc' ? reportBucket(r.submitted_at) : r.period;  // misc = its submission month
}
...
async function reportData(period) {
  // Non-misc match by their own period; misc by submission date; anything with a manual override too.
  const all = (await q(`SELECT s.ref_no, s.form_type, s.total_amount, s.final_at, s.final_by_name, s.pdf_token,
      s.period, s.submitted_at, s.report_period_override, e.name AS emp_name, e.emp_no
    FROM expense_submissions s JOIN employees e ON e.id=s.employee_id
    WHERE s.status='approved' AND s.paid_at IS NULL AND (s.period=$1 OR s.form_type='misc' OR s.report_period_override IS NOT NULL)`, [period])).rows;
  const rows = all
    .filter(r => effectivePeriod(r) === period)
    .map(({ period: _p, submitted_at: _s, report_period_override: _o, ...r }) => r)
    .sort((a, b) => (a.emp_name || '').localeCompare(b.emp_name || '') || (a.form_type || '').localeCompare(b.form_type || ''));
...
module.exports = { reportData, buildReportBuffer, reportFileName, cycleLabel, reportLabel, signLink, verifyLink };
