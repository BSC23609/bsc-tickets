...
async function reportData(period) {
  const rows = (await q(`SELECT s.ref_no, s.form_type, s.total_amount, s.final_at, s.final_by_name,
      e.name AS emp_name, e.emp_no
    FROM expense_submissions s JOIN employees e ON e.id=s.employee_id
    WHERE s.status='approved' AND s.period=$1
    ORDER BY e.name, s.form_type`, [period])).rows;
