...
const TEMPLATES = {
  ...
  dailyReport:  'daily_ticket_report2',
  myReport:     'my_ticket_report',
  document:     'ticket_document',
  external:     'ticket_external',
  welcome:      'employee_welcome',
};
...
const notify = {
  ...
  myReport: (to, r) => sendTemplate(to.phone, TEMPLATES.myReport, {
    ...
    total: r.total, open: r.open, closed: r.closed, q: `${r.dateISO}_${r.empId}` }),

  // New-employee onboarding / welcome. Sent once on add (and on demand via the
  // admin "Welcome" button). Variables: name (first name), empno, password.
  // Static "Open BSC Portal" URL button -> https://tickets.bharatsteels.in
  welcome: (emp, password) => sendTemplate(emp.phone, TEMPLATES.welcome, {
