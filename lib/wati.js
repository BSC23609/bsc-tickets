...
const OUTPASS_TPL = {
  ...
  overdue:  process.env.WATI_OVERDUE_TPL || 'outpass_overdue',   // gatepass not returned on time
};
...
const EXPENSE_TPL = {
  ...
  cv_pending:  'conveyance_pending',   // daily digest to a manager sitting on pending trips
};
...
async function sendTemplate(phone, templateName, params) {
  ...
  try {
    ...
    if (!r.ok) { warn(`${templateName} -> ${r.status} ${bodyText.slice(0, 300)}`); return false; }
...
const notify = {
  ...
  outpass: {
    ...
    overdue: (to, o) => sendTemplate(to.phone, OUTPASS_TPL.overdue, {
      name: to.name || 'Sir', employee: o.employee, ref: o.ref_no,
      out_time: o.out_time || '-', expected: o.expected || '-',
