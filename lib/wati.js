...
const notify = {
  ...
  // Local Conveyance → reporting-manager one-tap approval
  conveyance: {
    request: (mgr, s) => sendTemplate(mgr.phone, EXPENSE_TPL.cv_request, {
      name: mgr.name, ref: s.ref_no, requester: s.emp_name,
      period: s.period_label, total: s.total_label, token: s.action_token }),
    approved: (emp, s) => sendTemplate(emp.phone, EXPENSE_TPL.cv_approved, {
      name: emp.name, ref: s.ref_no, period: s.period_label, approver: s.approver_name, token: s.pdf_token }),
    rejected: (emp, s) => sendTemplate(emp.phone, EXPENSE_TPL.cv_rejected, {
      name: emp.name, ref: s.ref_no, period: s.period_label, approver: s.approver_name }),
  },

  // Payment-approval chain (HR → final approver → accounts)
  expense: {
    submitted:   (hr, s)  => sendTemplate(hr.phone,  CHAIN_TPL.submitted,    { name: hr.name,  ref: s.ref_no, requester: s.emp_name, form: s.form_label, period: s.period_label, total: s.total_label }),
    finalReview: (ap, s)  => sendTemplate(ap.phone,  CHAIN_TPL.final_review, { name: ap.name,  ref: s.ref_no, requester: s.emp_name, form: s.form_label, period: s.period_label, total: s.total_label }),
    returned:    (emp, s) => sendTemplate(emp.phone, CHAIN_TPL.returned,     { name: emp.name, ref: s.ref_no, form: s.form_label, period: s.period_label, stage: s.stage_label, reason: s.reason }),
