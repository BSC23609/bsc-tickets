...
const TEMPLATES = {
  ...
  password_otp: process.env.WATI_OTP_TPL || 'otp_password2',
...
const OUTPASS_TPL = {
  ...
  returnReminder: process.env.WATI_RETURN_REMINDER_TPL || 'gatepass_return_reminder2', // → requester, log your return (has "I'm Back" button)
};
...
const notify = {
  // Password reset code to the employee's own WhatsApp.
  // Authentication template (otp_password2): a single positional variable {{1}} = the code,
  // which also feeds the "Copy code" button.
  passwordOtp: (to, otp) => sendTemplate(to.phone, TEMPLATES.password_otp, { '1': String(otp) }),
  ot: {
  ...
  outpass: {
    ...
    returnReminder: (to, o) => sendTemplate(to.phone, OUTPASS_TPL.returnReminder, {
      ...
      // The "I'm Back" button URL variable — fill it under both names so the template can use
      // {{1}} or {{token}} in its button URL (https://tickets.bharatsteels.in/gpr/{{...}}).
      '1': o.token || o.return_token || '-', token: o.token || o.return_token || '-' }),
  },
