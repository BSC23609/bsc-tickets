...
async function nextRefNo() {
  ...
  const prefix = `TKT/BSC/${datePart}/`;
  const { rows } = await q(
    `SELECT COALESCE(MAX(NULLIF(split_part(ref_no, '/', 4), '')::int), 0) AS mx
     FROM tickets WHERE ref_no LIKE $1`,
    [`${prefix}%`]
  );
  const serial = String(rows[0].mx + 1).padStart(3, '0');
...
async function nextOutpassRefNo() {
  ...
  const dd = String(now.getDate()).padStart(2, '0');
  const datePart = `${yy}${mm}${dd}`;
  const prefix = `OGP/BSC/${datePart}/`;
  // Use the highest existing suffix (not COUNT) so deleting a pass never makes us reuse a number.
  const { rows } = await q(
    `SELECT COALESCE(MAX(NULLIF(split_part(ref_no, '/', 4), '')::int), 0) AS mx
     FROM outpass_requests WHERE ref_no LIKE $1`,
    [`${prefix}%`]);
